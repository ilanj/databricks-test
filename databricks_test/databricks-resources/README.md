...
The databricks CLI bundles top level is defined by file `databricks_test/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`databricks_test/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`databricks_test/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`databricks_test/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`databricks_test/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`databricks_test/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-databricks-test-model     | /dev-databricks-test-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest develop content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-databricks-test-model | /staging-databricks-test-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest master content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-databricks-test-model    | /prod-databricks-test-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the develop branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-databricks-test-model    | /test-databricks-test-experiment    |
...
2. Complete the `TODO` in `databricks_test/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Resources will be deployed to the prod workspace on pushing code to the `master` branch.
...
Following this, update the batch_inference_job base parameters in `databricks_test/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `databricks_test/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-databricks-test-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-databricks-test-batch-inference-job`
that runs the notebook under `databricks_test/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-databricks-test-batch-inference-job`.
...
`test-databricks-test-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-databricks-test-batch-inference-job`.
...
When you about to cut a release, you can create and merge a PR to merge changes from develop to master. Continuous deployment automation will deploy `prod` resources to the prod workspace.
