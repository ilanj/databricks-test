...
[ML Config](../databricks_test/databricks-resources/README.md)  guides to make changes to ML code or deployed jobs.
...
```
git add README.md docs .gitignore databricks_test/databricks-resources/README.md
...
* You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-3830983824680797.17.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-3830983824680797.17.azuredatabricks.net#setting/accounts). If
...
* [databricks-dev.yaml](../databricks_test/training/profiles/databricks-dev.yaml): specify recipe configs to use in dev workspace
* [databricks-staging.yaml](../databricks_test/training/profiles/databricks-staging.yaml): specify recipe configs to use in recurring model training and batch inference
  jobs that run in the staging workspace
* [databricks-prod.yaml](../databricks_test/training/profiles/databricks-prod.yaml) specify recipe configs to use in recurring model training and batch inference
  jobs that run in the prod workspace
* [databricks-test.yaml](../databricks_test/training/profiles/databricks-test.yaml): specify recipe configs to use in integration tests(CI)
...
Create and push a release branch called `master` off of the `develop` branch of the repository:
```
...
For future ML code changes, iterate against the `develop` branch and regularly deploy your ML code from staging to production by merging code changes from the `develop` branch into the `master` branch.
## Deploy ML resources and enable production jobs
Follow the instructions in [databricks-test/databricks-resources/README.md](../databricks_test/databricks-resources/README.md) to deploy ML resources
and production jobs.
...
[ML pull request guide](ml-pull-request.md) and [ML resource config guide](../databricks_test/databricks-resources/README.md)  to propose, test, and deploy
